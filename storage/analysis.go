...
func (stats *GameStatistics) ToDiscordEmbed(combinedID string, sett *GuildSettings) *discordgo.MessageEmbed {
	...
	fields = append(fields, &discordgo.MessageEmbedField{
		...
		Value:  stats.GameStartTime.Add(time.Duration(sett.GetTimeZone()*60) * time.Minute).Format("Jan 2, 3:04 PM"),
		Inline: true,
	})
	fields = append(fields, &discordgo.MessageEmbedField{
		Name: sett.LocalizeMessage(&i18n.Message{
		...
		Value:  stats.GameEndTime.Add(time.Duration(sett.GetTimeZone()*60) * time.Minute).Format("3:04 PM"),
		Inline: true,
	})
	fields = append(fields, &discordgo.MessageEmbedField{
		Name: sett.LocalizeMessage(&i18n.Message{
